...
#         n = len(self.data)
#         if n == 0:
#             raise IndexError("Dataset is empty after filtering. Check fg/bg/mask matching and thresholds.")
#         # 最多尝试 32 次顺延，避免无限循环
#         for t in range(min(32, n)):
#             j = (idx + t) % n
#             item = self.get_sample(j)
#             if item is not None:
#                 return item
#         # 给出可诊断的信息
#         raise IndexError("No valid sample found after 32 attempts. Consider relaxing check_mask_area or mask_score.")
...
#     #     H, W = mask.shape[0], mask.shape[1]
#     #     ratio = mask.sum() / (H * W)
#     #     if ratio > 0.2000 * 0.2000 or ratio < 0.0005 * 0.0005:
#     #         return False
#     #     else:
#     #         return True
#     # def check_mask_area(mask, min_ratio=1e-4, max_ratio=0.20):
#     #     H, W = mask.shape[:2]
#     #     area = float(mask.sum())
#     #     ratio = area / (H * W)
#     #     return (min_ratio <= ratio <= max_ratio)
#     def check_mask_area(self, mask):
#         # 如果是 PIL Image，就转 numpy
...
#         # if not self.check_mask_area(ref_mask):
#         #     print(f"Warning: Invalid mask found, skipping {ref_image}")
#         # return None  # 或者你可以选择返回一个默认的有效值
...
#         assert self.check_region_size(ref_mask, ref_box_yyxx, ratio = 0.00005, mode = 'min') == True
...
class BaseDataset(Dataset):
    ...
    def process_pairs(self, ref_image, ref_mask, tar_image, tar_mask, max_ratio = 0.9):
        # --- 检查 mask 分数 ---
        if mask_score(ref_mask) <= 0.90:
            return None
        if not self.check_mask_area(ref_mask):
            return None
        if not self.check_mask_area(tar_mask):
            return None

        # ========= Reference ===========
        ref_box_yyxx = get_bbox_from_mask(ref_mask)
        if not self.check_region_size(ref_mask, ref_box_yyxx, ratio = 0.00005, mode = 'min'):
            return None
        ...
        if not self.check_region_size(tar_mask, tar_box_yyxx, ratio = max_ratio, mode = 'max'):
            return None
