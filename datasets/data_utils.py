...
def mask_score(mask):
    ...
    if not cnts:
        return 0.0
    areas = np.array([cv2.contourArea(c) for c in cnts], dtype=float)
    return float(areas.max() / (areas.sum() + 1e-6))
...
def pad_to_square_ori(image, pad_value = 255, random = False):
    H,W = image.shape[0], image.shape[1]
    if H == W:
        return image
...
def pad_to_square(image, pad_value=255, random=False):
    """
    将一个 2D (HxW) 或 3D (HxWxC) 的图像填充成正方形。
    """
    # 检查输入是否有效
    ...
    # 根据图像的维度动态构建填充参数
    if image.ndim == 3:  # 对于 3D 图像 (H, W, C)
        if H > W:
            # 填充宽度 (第二个维度)
            pad_param = ((0, 0), (padd_1, padd_2), (0, 0))
        else:
    ...
    elif image.ndim == 2:  # 对于 2D 图像 (H, W)
        if H > W:
            # 填充宽度 (第二个维度)
        ...
        # 对于其他维度的数组，直接返回，避免出错
        print(f"Warning: pad_to_square received an image with unsupported dimensions: {image.shape}")
        return image
